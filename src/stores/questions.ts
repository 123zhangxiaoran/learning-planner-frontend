...
const GENERATING_KEY = 'questions-generating' // 正在生成的任务UUID
...
// 从localStorage获取正在生成的任务UUID
const getGeneratingTaskIdFromStorage = (): string | null => {
  return localStorage.getItem(GENERATING_KEY)
}
...
export const useQuestionsStore = defineStore('questions', () => {
  ...
  // 正在生成的任务UUID（从localStorage恢复）
  const generatingTaskId = ref<string | null>(getGeneratingTaskIdFromStorage())
  console.log('正在生成的任务ID:', generatingTaskId.value)
  ...
  // 设置正在生成状态（存储任务UUID）
  const setGenerating = (taskId: string) => {
    generatingTaskId.value = taskId
    localStorage.setItem(GENERATING_KEY, taskId)
  }
